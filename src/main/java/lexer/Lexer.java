...
public class Lexer {
    ...
    try {
      ...
      while (true) {
        ...
        String p = "L: " + tok.getLeftPosition() +
            ...
            tok.getKind() + " ";
          if ((tok.getKind() == TokenTypes.Identifier) ||
              (tok.getKind() == TokenTypes.INTeger)) {
              p += tok.toString();
          }
  ...
  public Token newIdToken(String id, int startPosition, int endPosition) {
    return new Token(startPosition, endPosition, Symbol.symbol(id, TokenTypes.Identifier));
  }
  ...
  public Token newNumberToken(String number, int startPosition, int endPosition) {
    return new Token(startPosition, endPosition,
        Symbol.symbol(number, TokenTypes.INTeger));
  }
  ...
  public Token makeToken(String s, int startPosition, int endPosition) {
    ...
    Symbol sym = Symbol.symbol(s, TokenTypes.BogusToken); // be sure it's a valid token
    if (sym == null) {
  ...
  public Token nextToken() { // ch is always the next char to process
    ...
    try {
      ...
      // token
      sym = Symbol.symbol(op, TokenTypes.BogusToken);
